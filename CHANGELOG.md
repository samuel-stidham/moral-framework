...
Rebuilt the harm policy's institutional rule as version 3.0.0, a MAJOR revision of the policy. Rule V3 previously made procedural regularity sufficient, so an institution following its own lawful procedures could impose any harm. Two earlier designs failed adversarial review and were discarded. The first bounded institutional authority by terms the adopting framework supplied, which left the check inside the party being checked. The second imported the whole Declaration, which no institution satisfies, so every arrest became resistible.

Harm imposed by an institution now violates no duty only when it is lawful, uses fair process, and stays within a floor. The floor states ten bars on acts, in the policy's own words. Nine name the gravest cases. They cover slavery, cruel treatment, arbitrary detention, legal personhood, equal protection, discrimination, retroactive punishment, belief, and care in custody. The tenth is general, barring an institution from arbitrarily stripping a person of what their life is built on. It reaches liberty, home, privacy, family, work, property, nationality, and public voice, so the floor is no longer criminal justice alone. Each bar names the institution, so no bar reaches private conduct. They draw on the Universal Declaration of Human Rights. The named bars cite Articles 2, 4, 5, 6, 7, 9, 11, 18, 19, and 25. The general bar cites Articles 3, 12, 13, 15, 16, 17, 20, 21, and 23.

The Definitions now define arbitrarily, which four bars rest on. A reason is arbitrary unless it would still hold were the person affected anyone else. Sincerity and legality do not save it. The discrimination bar keeps every ground the Declaration lists. The belief bar covers practice, and the seeking, receiving, and sharing of information, and excludes threats, deception, and calls to violence. The slavery bar excepts ordinary duties owed by everyone, and ordinary work asked of the sentenced. Labor imposed as the punishment itself is servitude. That exception is not drawn from the Declaration, and the Lineage says so.

No institution's own law moves the floor, and no permission in the policy reaches harm it bars. The custody bar is the sole exception, since care validly refused is not withheld. The Two Tracks index rule no longer reaches Rule V3 or Rule V1's institutional bars. Their terms are judged from outside the institution.

Fair process means the institution's own lawful procedures. Before punishment that deprives liberty it also means a public hearing before an impartial tribunal. It means notice, a real answer, and the presumption of innocence. A competent person may waive the hearing. Those come from Articles 10 and 11. The Definitions now define person, anchored to every human being, so the term left the delegation list. Fair process left it too, and five terms remain.

Rule V1 bars an institution from putting a person to death as punishment, and no consent makes a punishment something else. Other institutional killing outside a live danger needs valid consent. The Declaration is silent on capital punishment, and the Lineage says so plainly.

The Danger definition keys its institutional exclusion to the act, not the institution. Institutional harm is answerable with force only where a responder perceives a breach of the floor. Failing the other conditions may breach the duty and licenses nothing. Harm the killing bar forbids stays qualifying whatever a responder perceives. Judgment and Law now states that the policy guides the reader's conduct and judges institutional acts without commanding institutions. It adds that the policy does not decide what one nation owes another, and licenses no one to seek out a danger. The Permitted harm definition states that unpermitted harm is no violation by itself. The Brief and the Purpose were rewritten to match.

Principle 1 now states support for the Declaration in full, all thirty articles, without reservation. It also notes that the policy's floor is narrower. The Preamble's first step gains the harm boundary as a constraint on its balancing. The principles introduction now places the harm boundary above all four principles, and says it outranks them. Principle 3 carries the reasoning for the killing bar. It also drops the last Non-Aggression era wording and restates the floor in the policy's terms.

`CLAUDE.md` and `AGENTS.md` gain a review standard, so adversarial review of these documents ends at good faith. `docs/RELEASING.md` now states how a policy revision maps to the framework version, and that only the framework version is tagged. The 2.4.1 entry below now records that it shipped inside the v2.5.0 release. Five rounds of adversarial review, seventeen reviewers in total, shaped the result. Added the United Nations (1948) reference to the policy, cited in the Person definition, Rule V3, and the Lineage. Corrected APA on the Skelton, Frowe and Parry, and Sander-Staudt entries. Split one over-length sentence in the consent definition. Restructuring the principle hierarchy makes this a MAJOR revision of the framework.
