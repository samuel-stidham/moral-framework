...
Rebuilt the harm policy's institutional rule as version 3.0.0, a MAJOR revision of the policy. Rule V3 previously made procedural regularity sufficient, so an institution following its own lawful procedures could impose any harm. Two earlier attempts failed adversarial review and were discarded. The first bounded institutional authority by terms the adopting framework supplied, which left the check inside the party being checked. The second imported the whole Declaration, which no institution satisfies, so every arrest became resistible. Rule V3 now states a floor of eight bars in the policy's own words. They draw on Universal Declaration of Human Rights Articles 2, 4, 5, 6, 7, 9, 10, 11, 18, 19, and 25. Each bar judges a single act, never the institution as a whole. No institution's own law moves the floor, and the Two Tracks index rule no longer reaches it. A bar yields where an institution limits a right by law, only to secure the rights and freedoms of others. That limit never reaches slavery, torture, legal personhood, or retroactive punishment. An institution qualifies only when it is lawful, uses fair process, and stays within the floor. Fair process means the institution's own lawful procedures, and a full hearing before any punishment. The Definitions now define person, anchored to every human being, so the term left the delegation list. Five terms remain delegated. Rule V1 bars an institution from putting a person to death as punishment, and no consent makes a punishment something else. Other institutional killing outside a live danger needs valid consent. The Declaration is silent on capital punishment, and the Lineage says so plainly. The Danger definition now keys its institutional exclusion to the act, not the institution. Harm that lawful process can still undo is not qualifying harm. Judgment and Law states that the policy governs only the reader's own conduct. It no longer bars private force outright, since the license track now turns on a live danger. The Permitted harm definition states that unpermitted harm is no violation by itself. The Brief was rewritten to match, and the Purpose no longer claims a single non-delegated standard. Principle 1 now states support for the Declaration in full, all thirty articles, without reservation. The Preamble's first step gains the harm boundary as a constraint on its balancing. Principle 3 carries the reasoning for the killing bar. It also drops the last Non-Aggression era wording and restates the floor in the policy's terms. Three rounds of adversarial review, nine reviewers in total, shaped the result. Added the United Nations (1948) reference to the policy. Corrected APA italics on the three encyclopedia entries in both files. Split one over-length sentence in the consent definition. This is a MINOR revision of the framework.
